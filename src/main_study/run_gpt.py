...
def gpt4_vision_call(prompt, base64_image, image_name, model="gpt-4-turbo"):
...
def main(version, model_name="gpt-4-turbo"):
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        for image_type in ['afforded_image', 'non-afforded_image', 'related_image']:
            ...
            if image_name in encoded_images_dict:
                ...
                result = gpt4_vision_call(text, base64_image, image_name)
            else:
    ...
    # Save to .csv
    SAVE_PATH = "data/processed/models/closed_models/{model}_{version}_affordances.csv".format(model = model_name, version = version)
